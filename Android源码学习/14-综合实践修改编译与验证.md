# 14 综合实践：修改 Framework、增量编译并验证 Activity 启动链路
...
修改后启动任意 Activity，预期 logcat 出现类似：
...
- 追源码调用链。
- 用现有 logcat、am、dumpsys、Perfetto 验证进程和生命周期。
- 不修改系统镜像。
...
这是本章推荐路线。它比直接替换真机 `framework.jar` 更容易恢复。
...
本章不要求你现在一定完成全量镜像；可以先做到 B 级模块编译，再迁移到合适 Linux 环境完成 C 级验证。
...
记录冷启动、再次启动、配置重建三种日志次数，并用 Task/lifecycle 解释。
...
故意把 `SystemClock` 拼错一次，观察 Soong/ninja 报错中：
...
移除实验改动，重新编译并验证 AospStudy 日志消失，同时确保用户原有 diff 不受影响。
...
如果这些问题能独立回答，你已经从“阅读源码”迈到“用实验验证源码”。
