...
```java
long[] dijkstra(int n, List<int[]>[] graph, int start) {
    ...
    while (!pq.isEmpty()) {
        ...
        for (int[] edge : graph[node]) {
            ...
            int weight = edge[1];
            if (weight < 0) {
                throw new IllegalArgumentException("Dijkstra 不支持负权边");
            }
...
Dijkstra 不能处理负权边，因为负权边会破坏“当前最小距离可以最终确定”的贪心前提。
上面使用 `long` 和 `Comparator.comparingLong`，是为了避免路径累加和比较器相减发生 `int` 溢出。

复杂度（邻接表 + 二叉堆）通常写作 `O((V + E) log V)`，常简化为 `O(E log V)`；
空间复杂度为 `O(V + E)`。
